...
public class BaseUnitController : MonoBehaviour
{
    ...
    // ---------- プロパティ ----------
    [SerializeField] private float _rotateSpeed = 1000f; // キャラクターの回転速度
    [SerializeField] private float _moveSpeed = 7.0f; // 移動速度
    ...
    private void Update()
    {
        ...
        // 移動量を保存
        _velocity = _cameraTransform.TransformDirection(new Vector3(horizontal, 0, vertical));
    ...
    protected virtual void Move()
    {
        if (_velocity.magnitude > 0)
        {
            ...
            // 移動量に速度を掛ける
            Vector3 moveVelocity = _velocity.normalized * _moveSpeed;
